...
const TabsWithIndicator = ({ view, onViewChange, viewButtons }: TabsProps) => {
  ...
  return (
    <div className="w-full">
      <div
        ref={wrapRef}
        className="relative flex bg-white/50 rounded-full p-1 gap-1 w-full"
      >
        {pos && (
          <div
        ...
        {viewButtons.map(({ key, label }, i) => (
          <Button
            ...
            className={`relative z-10 flex-1 rounded-full text-xs sm:text-sm font-medium transition-smooth active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 px-3 sm:px-4 py-1.5 sm:py-2 text-center ${
              view === key
